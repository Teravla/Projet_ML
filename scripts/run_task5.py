...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Taille des images (défaut: 64)"
    )
...
def charger_modele(model_type: str, input_shape, num_classes: int):
    ...
    if model_type == "logreg":
        ...
        model_logits = build_cnn_classifier(input_shape=input_shape, num_classes=num_classes)
...
def main():
    ...
    print(f"Cas ambigus : {stats_secu['n_cas_ambigus']} ({stats_secu['taux_ambigus']:.1%})")
    ...
    print(f"  URGENTE : {stats_triage['n_urgente']} ({stats_triage['taux_urgente']:.1%})")
    print(f"  ELEVEE  : {stats_triage['n_elevee']} ({stats_triage['taux_elevee']:.1%})")
    print(f"  NORMALE : {stats_triage['n_normale']} ({stats_triage['taux_normale']:.1%})")
    print(f"  ROUTINE : {stats_triage['n_routine']} ({stats_triage['taux_routine']:.1%})")
